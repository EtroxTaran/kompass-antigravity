...
describe('LexwareSyncService', () => {
  ...
  describe('exportContractsToLexware', () => {
    it('should export signed contracts with recent modifications', async () => {
      ...
      exportService.exportLexwareContractsCsv.mockReturnValue(
        Buffer.from('csv'),
      );
      ...
      expect(contractRepository.findByStatus).toHaveBeenCalledWith('signed', {
        limit: 1000,
      });
      expect(exportService.exportLexwareContractsCsv).toHaveBeenCalledWith(
        contracts,
      );
  ...
  describe('importPaymentsFromLexware', () => {
    it('should process payment csv', async () => {
      ...
      (fs.readFileSync as jest.Mock).mockReturnValue(
        'InvoiceNumber;CustomerNumber;Date;Due;Gross;PayDate;PayAmount;Empty;Paid\nR-1001;123;;;;2025-01-01;100;;Paid',
      );

      invoiceRepository.findByInvoiceNumber.mockResolvedValue({
        _id: '1',
        status: 'sent',
      ...
      expect(invoiceRepository.findByInvoiceNumber).toHaveBeenCalledWith(
        'R-1001',
      );

...
export class LexwareSyncService implements OnModuleInit {
    ...
    async getSyncStatus() {
        ...
        return {
            ...
            files: [...exports, ...imports].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()),
            exportPath: path.resolve(this.EXPORT_PATH),
            importPath: path.resolve(this.IMPORT_PATH)
        };
    }

    async exportContractsToLexware(): Promise<{ count: number; filename: string }> {
        ...
        const result = await this.contractRepository.findByStatus('signed', { limit: 1000 });
        ...
        const contracts = result.data.filter(c => c.modifiedAt && c.modifiedAt > sevenDaysAgo);
    ...
    async importPaymentsFromLexware() {
        ...
        const files = fs.readdirSync(this.IMPORT_PATH).filter(f => f.endsWith('.csv') && !f.endsWith('.processed'));
        let totalImported = 0;
        ...
        for (const file of files) {
            ...
            const processLine = async (line: string) => {
                ...
                if (invoiceNumber && invoiceNumber.startsWith('R-')) { // Assuming Invoice No starts with R- or INV-
                    try {
                        // Find Invoice
                        ...
                        if (invoice) {
                            ...
                            if (invoice.status !== 'paid' && paymentStatus.toLowerCase() === 'paid') {
                                await this.invoiceRepository.update(invoice._id, { status: 'paid' }, 'system');
                                totalImported++;
                            }
                        }
                    } catch (e) {
                        this.logger.error(`Failed to process payment for ${invoiceNumber}`, e);
                    }
                }
            };

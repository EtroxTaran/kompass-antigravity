...
describe('PurchaseOrderService', () => {
  ...
  describe('submitForApproval', () => {
    it('should return error if not draft', async () => {
      ...
      await expect(service.submitForApproval('1', 'user1')).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('approve', () => {
    ...
    it('should fail > 1000 if NO BUCH/GF role', async () => {
      ...
      await expect(service.approve('1', 'user', ['USER'])).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('should fail > 10000 if BUCH role', async () => {
      ...
      await expect(service.approve('1', 'buch', ['BUCH'])).rejects.toThrow(
        ForbiddenException,
      );
    });

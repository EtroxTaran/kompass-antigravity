...
import { MaterialRepository, Material, SupplierPrice } from './material.repository';
...
export class MaterialService {
    ...
    async findAll(options: { page?: number; limit?: number; search?: string; category?: string } = {}) {
        if (options.search) {
            try {
                const searchResults = await this.searchService.search('materials', options.search, {
                ...
                const materials = await Promise.all(ids.map(id => this.materialRepository.findById(id)));
                return {
    ...
    async create(dto: CreateMaterialDto, user: { id: string; email?: string }): Promise<Material> {
        // Validate unique item number
        const existing = await this.materialRepository.findByItemNumber(dto.itemNumber);
        if (existing) {
        ...
        const newMaterial = await this.materialRepository.create(materialData as Partial<Material>, user.id, user.email);
        this.indexMaterial(newMaterial);
        return newMaterial;
    }

    async update(id: string, dto: UpdateMaterialDto, user: { id: string; email?: string }): Promise<Material> {
        const existing = await this.findById(id);

        // Validate unique item number if changing
        if (dto.itemNumber && dto.itemNumber !== existing.itemNumber) {
            const duplicate = await this.materialRepository.findByItemNumber(dto.itemNumber);
            if (duplicate) {
                throw new ConflictException({
                    type: 'https://api.kompass.de/errors/conflict',
                    title: 'Conflict',
        ...
        const updated = await this.materialRepository.update(id, { ...dto, ...priceStats } as Partial<Material>, user.id, user.email);
     ...
     */
    async addSupplierPrice(materialId: string, dto: AddSupplierPriceDto, user: { id: string; email?: string }): Promise<Material> {
        const material = await this.findById(materialId);
        const supplierPrices = [...(material.supplierPrices || [])];

        // Check if supplier already exists
        const existingIndex = supplierPrices.findIndex(p => p.supplierId === dto.supplierId);
     ...
     */
    async removeSupplierPrice(materialId: string, supplierId: string, user: { id: string; email?: string }): Promise<Material> {
        const material = await this.findById(materialId);
        const supplierPrices = (material.supplierPrices || []).filter(p => p.supplierId !== supplierId);
     ...
     */
    private calculatePriceStats(supplierPrices?: { unitPrice: number }[]): { averagePrice?: number; lowestPrice?: number } {

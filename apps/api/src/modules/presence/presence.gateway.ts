  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  handleDisconnect(client: Socket) {
    ...
    if (rooms) {
      rooms.forEach((room) => {
        this.server
          .to(room)
          .emit('presence:update', this.presenceService.getRoomUsers(room));
      });
    }
  }
  ...
  handleJoin(
    @MessageBody()
    data: { room: string; user: { userId: string; username: string } },
    @ConnectedSocket() client: Socket,
  ) {
    // this.logger.log(`User ${data.user.username} joining ${data.room}`);
    client.join(data.room);
    const users = this.presenceService.addUserToRoom(
      client.id,
      data.room,
      data.user,
    );

...
describe('PresenceService', () => {
  ...
  describe('addUserToRoom', () => {
    ...
    it('should handle adding multiple users to the same room', () => {
      service.addUserToRoom('socket-1', 'room-1', {
        userId: 'user-1',
        username: 'User 1',
      });
      const users = service.addUserToRoom('socket-2', 'room-1', {
        userId: 'user-2',
        username: 'User 2',
      });
    ...
    it('should update existing user in room (e.g. reconnect)', () => {
      ...
      const users = service.addUserToRoom('socket-1', 'room-1', {
        userId: 'user-1',
  ...
  describe('removeUserFromRoom', () => {
    ...
    it('should return empty array if room does not exist', () => {
      const result = service.removeUserFromRoom(
        'socket-1',
        'non-existent-room',
      );
  ...
  describe('handleDisconnect', () => {
    it('should remove user from all rooms on disconnect', () => {
      ...
      service.addUserToRoom('socket-1', 'room-2', {
        userId: 'user-1',
        username: 'User 1',
      });
  ...
  describe('updateHeartbeat', () => {
    it('should update lastHeartbeat for a user', () => {
      ...
      expect(usersAfter[0].lastHeartbeat.getTime()).toBeGreaterThanOrEqual(

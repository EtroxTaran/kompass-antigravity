...
describe('ExpenseService', () => {
    ...
    describe('create', () => {
        ...
        it('should throw BadRequestException if amount > 150 and no receipt', async () => {
            const dto = { amount: 160, description: 'Expensive Lunch', receiptUrl: undefined };
            await expect(service.create(dto as any, 'user1')).rejects.toThrow(BadRequestException);
        });

        it('should create expense > 150 if receipt provided', async () => {
            expenseRepository.create.mockResolvedValue(mockExpense);
            const dto = { amount: 160, description: 'Expensive Lunch', receiptUrl: 'http://url' };
            const result = await service.create(dto as any, 'user1');
            expect(result).toEqual(mockExpense);
        });
    });
    ...
    describe('approve', () => {
        it('should approve expense and update project cost', async () => {
            ...
            expenseRepository.update.mockResolvedValue({ ...mockExpense, status: 'approved' });

            const result = await service.approve('exp123', 'admin');

            expect(expenseRepository.update).toHaveBeenCalledWith('exp123', { status: 'approved', approvedBy: 'admin', approvedAt: expect.any(String) }, 'admin');
            expect(projectService.updateActualCost).toHaveBeenCalledWith('proj123', 'expenses', 200, 'admin');
            expect(result.status).toBe('approved');
        });

        it('should throw NotFoundException if expense not found', async () => {
            expenseRepository.findById.mockResolvedValue(null);
            await expect(service.approve('badId', 'admin')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('reject', () => {
        it('should reject expense', async () => {
            expenseRepository.findById.mockResolvedValue(mockExpense);
            expenseRepository.update.mockResolvedValue({ ...mockExpense, status: 'rejected' });

            const result = await service.reject('exp123', 'Too vague', 'admin');

            expect(expenseRepository.update).toHaveBeenCalledWith('exp123', { status: 'rejected', rejectionReason: 'Too vague', rejectedBy: 'admin', rejectedAt: expect.any(String) }, 'admin');

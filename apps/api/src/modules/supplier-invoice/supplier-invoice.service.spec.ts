...
describe('SupplierInvoiceService', () => {
    ...
    repo = {
      create: jest
        ...
        .mockImplementation((dto) =>
          Promise.resolve({ ...dto, _id: '1', version: 1 }),
        ) as any,
    };
    poService = {
      findOne: jest.fn(),
  ...
  describe('validate3WayMatch', () => {
    it('should fail if no PO found', async () => {
      ...
      (poService.findOne as jest.Mock).mockRejectedValue(
        new Error('Not found'),
      );
    ...
    it('should pass if PO matches, delivery exists, and variance <= 10%', async () => {
      ...
      (deliveryService.findByPurchaseOrder as jest.Mock).mockResolvedValue(
        deliveries,
      );
    ...
    it('should fail auto-approval if no delivery', async () => {
      ...
      expect(result.flags).toContain(
        'No delivered Goods Receipt found for this PO',
      );
    });
  });

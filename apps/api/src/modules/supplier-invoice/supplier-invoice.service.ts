...
export class SupplierInvoiceService {
  ...
  async findAll(supplierId?: string): Promise<SupplierInvoice[]> {
    if (supplierId) {
      return (
        await this.supplierInvoiceRepository.findBySelector({ supplierId })
      ).data;
    }
  ...
  async approve(id: string, userId: string): Promise<SupplierInvoice> {
    ...
    const updatedInvoice = await this.supplierInvoiceRepository.update(
      id,
      invoice,
      userId,
    );
    ...
    const costType = invoice.matchValidation?.deliveryMatch
      ? 'material'
      : 'subcontractor';
    await this.projectService.updateActualCost(
      invoice.projectId,

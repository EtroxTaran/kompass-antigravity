...
export class RfqService {
    ...
    async create(dto: CreateRfqDto, user: { id: string }): Promise<RequestForQuote> {
    ...
    async send(id: string): Promise<RequestForQuote> {
        ...
        const docDefinition: any = {
            content: [
                ...
                { text: `Deadline: ${new Date(rfq.responseDeadline).toLocaleDateString()}`, margin: [0, 0, 0, 20] },
                { text: 'Specifications', style: 'subheader' },
                { text: rfq.specifications, margin: [0, 0, 0, 10] },
                { text: `Quantity: ${rfq.quantity} ${rfq.unit}`, margin: [0, 0, 0, 10] },
            ],
            styles: {
        ...
        return this.repository.update(id, { status: RfqStatus.SENT }, rfq.createdBy, 'system');
    }

    async recordQuote(rfqId: string, dto: RecordQuoteDto): Promise<RequestForQuote> {
        const rfq = await this.findById(rfqId);

        if (rfq.status === RfqStatus.AWARDED || rfq.status === RfqStatus.CANCELLED) {
            throw new BadRequestException('Cannot record quotes for awarded or cancelled RFQs');
        }
        ...
        return this.repository.update(rfqId, {
            quotes,
            status: RfqStatus.QUOTES_RECEIVED
        }, 'system', 'system');
    }

    async awardQuote(rfqId: string, quoteId: string, user: { id: string, email: string }): Promise<RequestForQuote> {
        const rfq = await this.findById(rfqId);

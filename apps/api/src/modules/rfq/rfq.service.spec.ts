  ...
  describe('awardQuote', () => {
    it('should award a quote and create contract', async () => {
      ...
      mockRepository.update.mockImplementation((id, data) =>
        Promise.resolve({ ...rfq, ...data }),
      );
      mockContractService.createFromOffer.mockResolvedValue({
        _id: 'contract1',
      });

      const result = await service.awardQuote('rfq1', 'q1', {
        id: 'user1',
        email: 'u@e.com',
      } as any);

...
import {
    ProjectSubcontractorRepository,
} from './project-subcontractor.repository';
import {
...
export class ProjectSubcontractorService {
    ...
    async assign(
        ...
        const remainingBudget = (project.budget || 0) - (project.actualTotalCost || 0);
        ...
        await this.supplierService.update(supplier._id, { activeProjectCount: activeCount }, user);
    ...
    async update(
        ...
        let completionPercentage = dto.completionPercentage ?? existing.completionPercentage;
        ...
        if (dto.completionPercentage === 100 && status !== SubcontractorStatus.Completed) {
            ...
            if (existing.status !== SubcontractorStatus.Completed) {
                const supplier = await this.supplierService.findById(existing.supplierId);
                if (supplier) {
                    const activeCount = Math.max(0, (supplier.activeProjectCount || 1) - 1);
                    await this.supplierService.update(supplier._id, { activeProjectCount: activeCount }, user);
                }
            }
        }

...
describe('ProjectSubcontractorService', () => {
    ...
    beforeEach(async () => {
        ...
        service = module.get<ProjectSubcontractorService>(ProjectSubcontractorService);
    });
    ...
    describe('assign', () => {
        ...
        it('should assign successfully within budget', async () => {
            (supplierService.findById as jest.Mock).mockResolvedValue({ _id: 'supp-1', status: 'Active', activeProjectCount: 0 });
            (projectService.findById as jest.Mock).mockResolvedValue({ _id: 'proj-1', budget: 5000, actualTotalCost: 0 });
            (repository.create as jest.Mock).mockResolvedValue({ _id: 'sub-1', ...dto, status: 'Planned' });
        ...
        it('should assign with warning if over budget', async () => {
            (supplierService.findById as jest.Mock).mockResolvedValue({ _id: 'supp-1', status: 'Active' });
            (projectService.findById as jest.Mock).mockResolvedValue({ _id: 'proj-1', budget: 500, actualTotalCost: 0 });
            (repository.create as jest.Mock).mockResolvedValue({ _id: 'sub-1', ...dto });
        ...
        it('should fail if supplier not active', async () => {
            (supplierService.findById as jest.Mock).mockResolvedValue({ _id: 'supp-1', status: 'Blacklisted' });

            await expect(service.assign(dto, mockUser)).rejects.toThrow(BadRequestException);
        });
    });
});

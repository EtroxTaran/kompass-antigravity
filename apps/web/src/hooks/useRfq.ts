...
export const useRfq = () => {
  ...
  const fetchRfqs = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to fetch RFQs";
      setError(message);
  ...
  const getRfq = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        const message =
          err instanceof Error ? err.message : "Failed to fetch RFQ";
        setError(message);
        toast({ title: "Error", description: message, variant: "destructive" });
        throw err;
      } finally {
  ...
  const sendRfq = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        const message =
          err instanceof Error ? err.message : "Failed to send RFQ";
        setError(message);
        toast({ title: "Error", description: message, variant: "destructive" });
        throw err;
      } finally {
  ...
  const recordQuote = useCallback(
    async (id: string, data: RecordQuoteDto) => {
      setLoading(true);
      setError(null);
      try {
        ...
        const message =
          err instanceof Error ? err.message : "Failed to record quote";
        setError(message);
        toast({ title: "Error", description: message, variant: "destructive" });
        throw err;
      } finally {
  ...
  const awardQuote = useCallback(
    async (id: string, quoteId: string) => {
      setLoading(true);
      setError(null);
      try {
        ...
        toast({
          title: "Success",
          description: "Quote awarded and contract created",
        });
        return rfq;
      } catch (err: unknown) {
        const message =
          err instanceof Error ? err.message : "Failed to award quote";
        setError(message);
        toast({ title: "Error", description: message, variant: "destructive" });
        throw err;
      } finally {

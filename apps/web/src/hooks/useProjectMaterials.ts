...
export function useProjectMaterials(projectId?: string) {
    ...
    const updateMaterial = async (id: string, data: Partial<ProjectMaterialRequirement>) => {
        if (!projectId) return;
        try {
    ...
    return { materials, loading, error, addMaterial, updateMaterial, deleteMaterial, refetch: fetchMaterials };
}

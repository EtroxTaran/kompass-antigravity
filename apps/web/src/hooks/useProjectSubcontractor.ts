...
import { ProjectSubcontractor, AssignSubcontractorDto, UpdateAssignmentDto, RateSubcontractorDto } from '@kompass/shared';
...
export function useProjectSubcontractor(projectId?: string) {
    ...
    const fetchSubcontractors = useCallback(async () => {
        ...
        setError(null);
        try {
            const data = await apiClient.projectSubcontractor.findByProject(projectId);
    ...
    const assignSubcontractor = async (dto: AssignSubcontractorDto) => {
        ...
        try {
            ...
            const result = await apiClient.projectSubcontractor.assign(projectId, dto);
    ...
    const updateAssignment = async (id: string, dto: UpdateAssignmentDto) => {
        ...
        try {
            const result = await apiClient.projectSubcontractor.update(projectId, id, dto);
            setSubcontractors(prev => prev.map(item => item._id === id ? result : item));
            return result;
        } catch (err: any) {
    ...
    const rateSubcontractor = async (id: string, dto: RateSubcontractorDto) => {
        if (!projectId) return;
        try {
            const result = await apiClient.projectSubcontractor.rate(projectId, id, dto);
            setSubcontractors(prev => prev.map(item => item._id === id ? result : item));
            return result;
        } catch (err: any) {
            throw err;

  ...
  return (
    <div className="container mx-auto py-6 max-w-4xl space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={() => { refresh(); triggerSync(); }}>
          <RefreshCw className={`mr-2 h-4 w-4 ${status === 'active' ? 'animate-spin' : ''}`} />
          Aktualisieren
        </Button>
      </div>
                ...
                <div className="space-y-4">
                  {tierQuotas.map((tier) => (
                    <div key={tier.tier} className="space-y-1">
                      <div className="flex justify-between text-sm">
                        <span className="capitalize">{tier.tier === 'onDemand' ? 'On-Demand (Pinned)' : tier.tier}</span>
                        <span className="text-gray-500">
                      ...
                      <p className="text-xs text-gray-400">
                        {tier.tier === 'essential' && 'Benutzerdaten, Kunden, Termine (Wird niemals gelöscht)'}
                        {tier.tier === 'recent' && 'Protokolle der letzten 30 Tage (Auto-Löschung bei Voll)'}
                        {tier.tier === 'onDemand' && 'Manuell gepinnte Dokumente'}
                      </p>
                    </div>
                  ))}
                </div>
          ...
          <CardContent>
            {pinnedIds.length === 0 ? (
              <div className="text-center py-8 text-gray-500 text-sm">
                ...
                <br />
                Nutzen Sie das <span className="inline-block px-1 bg-gray-100 rounded border">📌</span> Icon um Inhalte verfügbar zu halten.
              </div>
            ) : (
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800 text-sm text-blue-800 dark:text-blue-200">
        ...
        <p>
          ...
          Verlauf (Recent Tier) wird stündlich aktualisiert.
          Nutzen Sie "Aktualisieren", um einen sofortigen Abgleich aller Daten zu erzwingen.
        </p>
      </div>
    </div>
  );

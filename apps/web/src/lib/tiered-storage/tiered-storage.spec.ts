...
describe('Tiered Storage Strategy', () => {
    ...
    describe('essentialFilter', () => {
        ...
        it('should return true for Todays Activity', () => {
            const doc = { type: 'activity', assignedTo: userId, scheduledAt: new Date().toISOString() };
            expect(essentialFilter(doc, req)).toBe(true);
        });
        ...
        it('should return false for Future Activity', () => {
            ...
            const doc = { type: 'activity', assignedTo: userId, scheduledAt: futureDate.toISOString() };
            expect(essentialFilter(doc, req)).toBe(false);
        });
    ...
    describe('evictLruDocuments', () => {
        ...
        it('should evict oldest non-essential documents to reach target bytes', async () => {
            const docOld = { _id: '1', type: 'note', modifiedAt: '2023-01-01T00:00:00Z', content: 'x'.repeat(100) };
            const docNew = { _id: '2', type: 'note', modifiedAt: '2023-12-01T00:00:00Z', content: 'x'.repeat(100) };
            const docEssential = { _id: '3', type: 'customer', owner: userId, modifiedAt: '2023-01-01T00:00:00Z' };
        ...
        it('should not evict essential documents even if old', async () => {
            const docEssential = { _id: '3', type: 'customer', owner: userId, modifiedAt: '2020-01-01T00:00:00Z' };
            mockAllDocs.mockResolvedValue({ total_rows: 1, rows: [{ doc: docEssential }] });

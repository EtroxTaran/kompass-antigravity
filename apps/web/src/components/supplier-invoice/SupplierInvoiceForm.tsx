...
export function SupplierInvoiceForm({ supplierId, projectId }: SupplierInvoiceFormProps) {
    const navigate = useNavigate();
    const { createInvoice } = useSupplierInvoice();
    const { register, control, handleSubmit, watch, setValue, formState: { errors } } = useForm<CreateSupplierInvoiceDto>({
        defaultValues: {
    ...
    // Calculate totals
    useEffect(() => {
        const net = lineItems.reduce((acc, item) => acc + (item.quantity * item.unitPrice), 0);
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-4">
                ...
                <h2 className="text-2xl font-bold tracking-tight">New Supplier Invoice</h2>
            </div>
                ...
                <Card>
                    ...
                    <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input id="supplierId" {...register('supplierId', { required: true })} />
                        </div>

                        <div className="space-y-2">
                            <Label htmlFor="projectId">Project ID</Label>
                            <Input id="projectId" {...register('projectId', { required: true })} />
                        </div>

                        <div className="space-y-2">
                            <Label htmlFor="contractId">Contract ID (Optional)</Label>
                            <Input id="contractId" {...register('contractId')} placeholder="Links to Purchase Order" />
                        </div>

                        <div className="space-y-2">
                            <Label htmlFor="invoiceDate">Invoice Date</Label>
                            <Input type="date" id="invoiceDate" {...register('invoiceDate', { required: true })} />
                        </div>
                    ...
                    <CardContent className="space-y-4">
                        {fields.map((field, index) => (
                            <div key={field.id} className="grid grid-cols-12 gap-2 items-end border-b pb-4">
                                ...
                                <div className="col-span-12 md:col-span-2 space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setValue(`lineItems.${index}.totalPrice` as const, qty * price);
                                        }}
                                    />
                                </div>
                                <div className="col-span-12 md:col-span-2 space-y-2">
                                ...
                                <div className="col-span-12 md:col-span-2 space-y-2">
                                    <Label>Total</Label>
                                    <Input type="number" readOnly {...register(`lineItems.${index}.totalPrice` as const)} className="bg-muted" />
                                </div>
                    ...
                    <CardContent className="pt-6">
                        <div className="flex flex-col items-end gap-2">
                            <div className="flex items-center gap-4">
                                ...
                                <Input className="w-32 text-right" readOnly {...register('netAmount')} />
                            </div>
                            <div className="flex items-center gap-4">
                                <span className="font-medium">Tax Rate (%):</span>
                                <Input className="w-32 text-right" type="number" {...register('taxRate')} />
                            </div>
                            <div className="flex items-center gap-4">
                                <span className="font-medium">Tax Amount:</span>
                                <Input className="w-32 text-right" readOnly {...register('taxAmount')} />
                            </div>
                            <div className="flex items-center gap-4 text-lg font-bold">
                                <span>Gross Amount:</span>
                                <Input className="w-32 text-right font-bold" readOnly {...register('grossAmount')} />
                            </div>
                        </div>
                    </CardContent>
                </Card>

    ...
    <div className="space-y-4">
                  ...
                  <TableCell>
                    {invoice.matchValidation && (
                      <div className="flex gap-1">
                        {invoice.matchValidation.poMatch ? (
                          <span className="text-green-500">PO</span>
                        ) : (
                          <span className="text-red-500">PO</span>
                        )}
                        <span>/</span>
                        {invoice.matchValidation.deliveryMatch ? (
                          <span className="text-green-500">DEL</span>
                        ) : (
                  ...
                  <TableCell className="text-right">
                    {invoice.paymentStatus === "Pending" && (
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={(e) => handleApprove(invoice._id, e)}
                      >
      ...
      >
        <DialogContent className="max-w-3xl">
          ...
          {selectedInvoiceId &&
            invoices?.find((i: SupplierInvoice) => i._id === selectedInvoiceId)
              ?.matchValidation && (
              <InvoiceMatchValidation
                validation={
                  invoices.find(
                    (i: SupplierInvoice) => i._id === selectedInvoiceId,
                  )!.matchValidation!
                }
              />
            )}
          {selectedInvoiceId &&
            !invoices?.find((i: SupplierInvoice) => i._id === selectedInvoiceId)
              ?.matchValidation && <p>No 3-way match data available.</p>}
        </DialogContent>
      </Dialog>
    </div>
  );

...
export function AtRiskProjectsWidget({ projects }: AtRiskProjectsWidgetProps) {
    ...
    const atRisk = projects.filter(
        p => (p.budgetStatus === 'Warning' || p.budgetStatus === 'Exceeded') && p.status === 'active'
    );
                        ...
                        atRisk.map((project) => (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-2">
                                    <span className={`text-xs font-bold px-2 py-1 rounded-full ${project.budgetStatus === 'Exceeded'
                                        ? 'bg-red-100 text-red-600'
                                        : 'bg-amber-100 text-amber-600'
                                        }`}>
                                        {project.budgetStatus === 'Exceeded' ? 'ÜBERZOGEN' : '> 80%'}
                                    </span>
                                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate(`/projects/${project._id}`)}>

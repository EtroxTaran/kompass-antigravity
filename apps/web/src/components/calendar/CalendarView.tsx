  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden flex flex-col h-[600px]">
      ...
      <ScrollArea className="flex-1">
        <div
          className="relative"
          style={{ height: (END_HOUR - START_HOUR) * 60 + 50 }}
        >
          {/* Time Slots */}
          {TIME_SLOTS.map((hour) => (
            <div
              key={hour}
              className="flex border-b border-border h-[60px] last:border-b-0"
            >
    ...
    <div className="border border-border rounded-lg overflow-hidden flex flex-col h-[600px]">
      ...
      <div className="grid grid-cols-7 border-b border-border ml-16">
        {days.map((day) => (
          <div
            key={day.toISOString()}
            className={`p-2 text-center border-r border-border last:border-r-0 ${isSameDay(day, new Date()) ? "bg-blue-50" : ""}`}
          >
            <div className="text-sm font-medium">
              {["So", "Mo", "Di", "Mi", "Do", "Fr", "Sa"][day.getDay()]}
            </div>
            <div
              className={`text-sm ${isSameDay(day, new Date()) ? "text-blue-600 font-bold" : "text-muted-foreground"}`}
            >
              {day.getDate()}
            </div>
          </div>
          ...
          <div className="absolute top-0 left-16 right-0 bottom-0 grid grid-cols-7">
            {days.map((day) => {
              ...
              const dayEvents = events.filter(
                (e) => isSameDay(new Date(e.startDate), day) && !e.allDay,
              );

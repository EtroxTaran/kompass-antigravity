...
export function StorageAlert() {
    const { storage, isStorageWarning, isStorageCritical, requestPersistentStorage } = useSyncStatus();
        ...
        >
            <div className="flex items-center gap-2">
                ...
                <AlertDescription>
                    {isStorageCritical ? (
                        <>
                            <strong>Speicher voll!</strong> Synchronisierung pausiert.
                            Bitte löschen Sie nicht benötigte Daten. ({usageMB} MB / {quotaMB} MB, {usagePercent}%)
                        </>
                    ) : (
                        <>
                            <strong>Speicher fast voll</strong> ({usagePercent}%).
                            Die Synchronisierung könnte bald pausieren.
                        </>
                    )}
                </AlertDescription>
            </div>

...
export function MaterialDetail({ material }: MaterialDetailProps) {
    ...
    const handleSelectSupplier = (supplierId: string, supplierName: string, unitPrice: number) => {
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div className="font-semibold text-muted-foreground">Kategorie</div>
                            <div>{material.category}</div>

                            <div className="font-semibold text-muted-foreground">Beschreibung</div>
                            <div>{material.description || '-'}</div>

                            <div className="font-semibold text-muted-foreground">Bevorzugter Lieferant</div>
                            <div
                                className={supplier ? "cursor-pointer text-primary hover:underline" : ""}
                                onClick={() => supplier && navigate(`/suppliers/${supplier._id}`)}
                            >
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div className="flex justify-between items-center">
                            <div className="text-sm font-semibold text-muted-foreground">Lagerbestand</div>
                            <div className="text-2xl font-bold">
                                {material.inStock ?? 0} <span className="text-sm font-normal text-muted-foreground">{unitLabel}</span>
                            </div>
                        </div>
                        <div className="flex justify-between items-center">
                            <div className="text-sm font-semibold text-muted-foreground">Einkaufspreis</div>
                            <div className="text-xl">
                                {material.purchasePrice.toLocaleString('de-DE', { style: 'currency', currency: material.currency })}
                            </div>
                        </div>
                        {material.averagePrice && (
                            <div className="flex justify-between items-center text-muted-foreground">
                                ...
                                <div className="text-sm">
                                    {material.averagePrice.toLocaleString('de-DE', { style: 'currency', currency: material.currency })}
                                </div>
                            </div>
                        )}
                    </CardContent>

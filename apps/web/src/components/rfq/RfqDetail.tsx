...
export function RfqDetail() {
  ...
  const getStatusBadge = (status: RfqStatus) => {
    switch (status) {
      ...
      case RfqStatus.QUOTES_RECEIVED:
        return (
          <Badge variant="default" className="bg-yellow-500 text-lg">
            Quotes Received
          </Badge>
        );
  ...
  const canRecordQuotes =
    rfq.status === RfqStatus.SENT || rfq.status === RfqStatus.QUOTES_RECEIVED;
  const canAward =
    rfq.status === RfqStatus.QUOTES_RECEIVED && (rfq.quotes?.length || 0) > 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          ...
          {rfq.status === RfqStatus.DRAFT && (
            <Button
              onClick={handleSend}
              disabled={loading || (rfq.invitedSuppliers?.length || 0) === 0}
            >
      ...
      {/* Alerts */}
      {rfq.status === RfqStatus.DRAFT &&
        (rfq.invitedSuppliers?.length || 0) === 0 && (
          <Alert variant="destructive">
            <AlertTriangle className="h-4 w-4" />
            <AlertTitle>No Suppliers Selected</AlertTitle>
            <AlertDescription>
              This RFQ has no suppliers selected. Please edit the RFQ to add
              suppliers before sending.
            </AlertDescription>
          </Alert>
        )}
      ...
      {isDeadlinePassed && rfq.status !== RfqStatus.AWARDED && (
        <Alert>
          ...
          <AlertDescription>
            The response deadline for this RFQ has passed. You can still record
            any late quotes.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {rfq.quotes?.length || 0} quotes received from{" "}
              {rfq.invitedSuppliers?.length || 0} invited suppliers
            </CardDescription>
          </CardHeader>
          <CardContent>
            <RfqComparisonTable

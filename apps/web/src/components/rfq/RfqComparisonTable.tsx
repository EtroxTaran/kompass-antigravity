...
export function RfqComparisonTable({ rfq, onAward }: RfqComparisonTableProps) {
  ...
  const lowestPrice = Math.min(
    ...quotes.map((q: SupplierQuote) => q.quotedPrice),
  );
  // Find the fastest delivery
  const fastestDelivery = Math.min(
    ...quotes.map((q: SupplierQuote) => q.deliveryDays),
  );
        ...
        <TableBody>
          {quotes.map((quote: SupplierQuote) => (
            <TableRow
              key={quote.id}
              className={
                quote.status === QuoteStatus.AWARDED ? "bg-green-50" : ""
              }
            >
              <TableCell className="font-medium">{quote.supplierId}</TableCell>
              <TableCell>
                <span
                  className={
                    quote.quotedPrice === lowestPrice

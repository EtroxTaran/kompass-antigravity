...
export function RfqQuoteForm({ invitedSupplierIds, onSubmit, loading }: RfqQuoteFormProps) {
    const [open, setOpen] = useState(false);
    const { suppliers } = useSuppliers();

    // Filter to only show invited suppliers
    const invitedSuppliers = suppliers.filter(
        (s: Supplier) => invitedSupplierIds.includes(s._id)
    );
                    ...
                    <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <FormField
                                control={form.control}
                                name="deliveryDays"
                                rules={{ required: 'Delivery days required', min: { value: 1, message: 'Must be at least 1 day' } }}
                                render={({ field }) => (
                                    <FormItem>

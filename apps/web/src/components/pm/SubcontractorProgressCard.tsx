...
export function SubcontractorProgressCard({ assignment }: Props) {
    ...
    const [progress, setProgress] = useState(assignment.completionPercentage || 0);
    const [updating, setUpdating] = useState(false);

    const handleUpdate = async () => {
        setUpdating(true);
        try {
            await updateAssignment(assignment._id, { completionPercentage: progress });
        } finally {
            setUpdating(false);
        }
    };
        ...
        <div className="bg-slate-50 p-4 rounded-lg">
            ...
            <div className="flex justify-between items-center text-xs text-gray-500 mb-2">
                ...
                {assignment.actualCost ?
                    <span className={assignment.budgetStatus === 'Exceeded' ? 'text-red-500 font-bold' : ''}>

...
# Keycloak ADM User Creation Script
# Creates an ADM (Außendienstmitarbeiter) user in Keycloak and CouchDB if it doesn't exist.
# Supports development, staging, and production environments.
#
# Usage:
#   ./scripts/keycloak-create-adm.sh
#
...
#   ADM_USER_EMAIL - ADM user email (default: adm@kompass.de)
#   ADM_USER_PASSWORD - ADM user password (default: Adm123!@#)
#   ADM_USER_DISPLAY_NAME - ADM user display name (default: Test ADM)
...
ADM_USER_EMAIL="${ADM_USER_EMAIL:-adm@kompass.de}"
ADM_USER_PASSWORD="${ADM_USER_PASSWORD:-Adm123!@#}"
ADM_USER_DISPLAY_NAME="${ADM_USER_DISPLAY_NAME:-Test ADM}"
...
ADM_USER_FIRST_NAME="${ADM_USER_FIRST_NAME:-Test}"
ADM_USER_LAST_NAME="${ADM_USER_LAST_NAME:-ADM}"

echo -e "${BLUE}👤 Keycloak ADM User Creation${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
main() {
  ...
  echo "  ADM User Email: ${ADM_USER_EMAIL}"
  echo "  Display Name: ${ADM_USER_DISPLAY_NAME}"
  echo ""
  ...
  echo -e "${YELLOW}🔍 Checking if ADM user exists...${NC}"
  
  encoded_email=$(echo "$ADM_USER_EMAIL" | sed 's/@/%40/g')
  ...
  if [ -n "$KEYCLOAK_USER_ID" ] && [ "$KEYCLOAK_USER_ID" != "null" ] && [ "$KEYCLOAK_USER_ID" != "" ]; then
    echo -e "${GREEN}✅ ADM user already exists in Keycloak${NC}"
    echo -e "${YELLOW}   User ID: ${KEYCLOAK_USER_ID}${NC}"
    
    # Ensure ADM role is assigned
    echo -e "${YELLOW}🔗 Ensuring ADM role is assigned...${NC}"
    assign_role_to_user "$ADMIN_TOKEN" "$KEYCLOAK_USER_ID" "ADM" || true
  else
    echo -e "${YELLOW}👤 ADM user does not exist, creating...${NC}"
    # Create user in Keycloak
    create_keycloak_user "$ADMIN_TOKEN" "$ADM_USER_EMAIL" "$ADM_USER_PASSWORD" "$ADM_USER_FIRST_NAME" "$ADM_USER_LAST_NAME" || exit 1
    
    # Get user ID (wait a moment for Keycloak to process)
    sleep 1
    KEYCLOAK_USER_ID=$(get_keycloak_user_id "$ADMIN_TOKEN" "$ADM_USER_EMAIL")
    ...
    # Assign ADM role
    assign_role_to_user "$ADMIN_TOKEN" "$KEYCLOAK_USER_ID" "ADM" || exit 1
  fi

  # Check if user exists in CouchDB
  if user_exists_in_couchdb "$ADM_USER_EMAIL"; then
    echo -e "${YELLOW}⚠️  ADM user already exists in CouchDB${NC}"
  else
    # Create user in CouchDB
    create_couchdb_user "$ADM_USER_EMAIL" "$ADM_USER_DISPLAY_NAME" "$KEYCLOAK_USER_ID" || exit 1
  fi

  echo ""
  echo -e "${GREEN}✅ ADM user setup completed successfully!${NC}"
  echo ""
  echo -e "${GREEN}📋 Summary:${NC}"
  echo "  Email: ${ADM_USER_EMAIL}"
  echo "  Password: ${ADM_USER_PASSWORD}"
  echo "  Display Name: ${ADM_USER_DISPLAY_NAME}"
  echo "  Role: ADM (Außendienstmitarbeiter)"

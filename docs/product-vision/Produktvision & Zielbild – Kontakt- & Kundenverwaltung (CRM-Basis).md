...
*Converted from: Produktvision & Zielbild – Kontakt- & Kundenverwaltung (CRM-Basis).pdf*  
...
**Domänenbeschreibung:** Die Domäne *Kontakt- & Kundenverwaltung* umfasst die **Verwaltung aller**
...
Lieferanten, Verbände) in *einer* Domäne konsistent abzubilden. Der Projektkontext deutet an, dass
...
. *Risiko:* anfängliche Verwirrung, wenn z.B. Vertrieb plötzlich Lieferanten in
der Kontaktliste sieht. *Mitigation:* Klare UI-Kennzeichnung nach Typ (Filteroption nach „Kunde“,
...
.
Zudem wird es eine **Merge-Funktion** geben, um bestehende Dubletten zusammenzuführen. *Risiko:*
Wenn Nutzer unsauber eingeben (z.B. Schreibvarianten), könnten Dubletten dennoch entstehen.
*Mitigation:* regelmäßige Datenpflege durch definierte Verantwortliche (siehe unten), sowie technisch
...
eigenständiges Kundenkonto geführt wird. **Annäherung aus Best Practice:** Ein *Lead* bleibt so lange
...
. Diese Umwandlung
wird im Tool per Klick unterstützt (siehe Anforderungen). *Risiko:* ohne klare Regeln könnten zu früh
Accounts angelegt werden oder umgekehrt Leads zu lange „vergessen“ werden. *Mitigation:*
...
. **Lösung:** Wir führen flexible Beziehungstypen ein, z.B. *„gehört zu“* , *„Mitglied von“* , *„empfohlen*
*durch“* etc., sodass ein Kontaktdatensatz Links zu anderen haben kann
. Damit kann man z.B.
bei einem Kunden sehen: gehört zu Franchise-Gruppe X, Mitglied im Regionalverband Y, Lieferant Z
hat Projekte für ihn ausgeführt, etc. *Offene Detailfrage:* **<Verband als eigener Kontakt?>** – Soll
...
Performance flüssig bleiben (quantifiziert: API P95 ≤1,5s, Listen-Laden ≤2s, siehe NFR_SPECIFICATION.md)
. *Mitigation:* Wir orientieren uns an modernen CRM-Oberflächen,
...
an Marketing meldete und Word-Profile erstellt werden – ein *langsamer, fehleranfälliger Prozess*
.
...
vermittelt hat. *Entscheidung:* Diese Feature ist enthalten, obwohl es in Standard-CRMs selten out-of-
...
nachvollziehbar sein – d.h. die *Akquisequelle und der verantwortliche Verkäufer* je Auftrag müssen
...
Unterstützung (Status “nicht löschbar, hat Finanzdaten”). *Kein* relevanter Bedarf der Buchhaltung ist
...
. Dies betrifft zwar eine mögliche *Service-/Ticket-Domäne* in der Zukunft, aber man kann es als
...
. Jede Anforderung wird mit einem kurzen *Abnahmekriterium* ergänzt, das beschreibt, wann sie
erfüllt ist. Außerdem klären wir *Nicht-Ziele* (bewusst Ausgegrenztes).
...
*Abnahme:* Ein neuer Kontakt lässt sich anlegen mit mindestens Name und Typ (z.B. “Firma” vs.
...
Lieferanten: zusätzlich angebotene Leistungen/Gewerke. *Abnahme:* In der Maske für einen Kontakt
...
. *Abnahme:* Öffnet man ein Kundenprofil, sieht
...
. *Abnahme:* Im Kontaktprofil gibt es einen Bereich “Dokumente”, in
...
*Abnahme:* Testcase: Benutzer versucht “Firma X GmbH” anzulegen, obwohl “X GmbH” existiert –
...
der direkten Firmenhierarchie. Insbesondere: *Mitgliedschaften* (Firma A ist Mitglied in Verband Y),
*Empfehlungen* (Kontakt Z hat uns Kontakt X vermittelt), oder generische Verknüpfungen zu Partnern/
Lieferanten (Lieferant Q beliefert Kunde A). Diese Relationen sollen im Kontaktprofil sichtbar sein.
*Abnahme:* Ein Kontaktprofil “Kunde A” zeigt z.B. “Mitglied im Verband XY” und “Empfohlen durch
...
Status haben (z.B. “neu”, “in Qualifizierung”). *Abnahme:* Benutzer kann Lead erfassen mit Name und
...
Doppelerfassen). *Abnahme:* Im Lead-Datensatz gibt es die Funktion “Lead umwandeln”; nach
...
Historie)
. *Abnahme:* Projekt anlegen mit Auswahl eines Kunden aus Kontaktliste ist möglich; im
...
werden erfasst, etwa *Erstelldatum des Leads* , *Konversionsdatum* , *Neukunde/Jahr* , etc., sodass KPI wie

14.

“Anzahl neue Leads/Monat, Konversionsquote Lead → Auftrag” berechnet werden können
.
*Abnahme:* Beispielsweise ein Report “Leads pro Monat” lässt sich mit den Daten erzeugen (Test durch
...
Kontakte). *Abnahme:* Admin kann für Rollen Rechte setzen, z.B. Rolle “Vertrieb” darf Kontakte
...
.
**DSGVO-Funktionen**
: Export und Löschung von Kontaktdaten. *Abnahme:* In der Admin-
...
betrieben
. *Abnahme:* Es existiert ein Änderungsprotokoll, wo nachvollziehbar ist, wenn z.B.
...
Jeder Standort hat eine eigene Lieferadresse, getrennt von der Rechnungsadresse. *Abnahme:*
...
*Abnahme:* Im Kundenstamm gibt es ein Pflichtfeld "Rechnungsadresse" und optional mehrere
...
**Entscheidungsrollen** versehen werden: *Decision Maker* (finale Entscheidungsbefugnis),
*Key Influencer* (starker Einfluss), *Recommender* (gibt Empfehlungen), *Gatekeeper*
(kontrolliert Zugang), *Operational Contact* (operative Ebene), *Informational* (nur informiert).
Zusätzlich können **funktionale Rollen** zugewiesen werden (z.B. Geschäftsführer,
Einkaufsleiter, Filialleiter). *Abnahme:* Beim Bearbeiten eines Kontakts gibt es Felder für
...
überschreitet. *Abnahme:* Im Kontaktprofil ist das Feld "Genehmigungslimit (€)" editierbar. Bei
...
doppelter Kontakte (Merge-Funktion mit Auswahl, welche Felder zu übernehmen sind). *Abnahme:*
...
. *Abnahme:* Bei Eingabe eines neuen Unternehmens
...
der Kundenstammdaten mit der FiBu-Software via REST API. *Abnahme:* Legt man im CRM einen neuen Kunden an,
...
Größenklassen etc.), um Kontakte noch gezielter zu segmentieren. *Abnahme:* Nutzer kann aus einer
...
View). *Abnahme:* Für einen gegebenen Kunden kann man eine Graph-Ansicht öffnen, die z.B. alle
...
Performance/UX dadurch verbessert werden kann. *Abnahme:* Außendienstler haben eine App im
...
auftauchen
. *Abnahme:* Wenn der Vertrieb z.B. eine Mail an kunde@firma.de sendet, wird diese
...
und automatisch einen neuen Kontakt zu erstellen (OCR). *Abnahme:* Foto einer Visitenkarte erzeugt
...
**Öffentliches CRM/Marketing-Tool:** Die Lösung ist *intern* fokussiert – Funktionen wie Massen-E-Mail-
...
von allen gerne genutzt wird – der *Single Source of Truth* für Kundenbeziehungen
.
...
**extern.** Wir arbeiten mit *einem* System statt fünf Insellösungen, was die Zusammenarbeit verbessert und
...
*Kundenkonto* wird einmal angelegt und dann in allen relevanten Modulen referenziert (Opportunity hat Feld
...
- *Lead-Handhabung:* neu eingehende Leads werden auf einem Dashboard angezeigt. Vertriebsinnendienst
...
- *Opportunity-Pipeline:* Vertriebsmeetings nutzen den Pipeline-Report aus dem System, man aktualisiert
Phasen/Abschlusswahrscheinlichkeiten direkt dort, statt in Excel.
- *Kundenkommunikation:* Jeder Kundenkontakt (Anruf, Meeting) wird möglichst direkt nach Gespräch als
...
- *Auftrag* *→* *Rechnung:* Sobald ein Projekt beauftragt ist, legt das System automatisch die vorgesehenen
  ...
  der Rechnung.
  - *Datenpflege-Prozess:* Alle 6 Monate wird automatisch eine Aufgabe “Kundendaten überprüfen” generiert für
...
**Kern des Designs** ist ein **zentrales Datenmodell** mit den Entitäten: *Kundenkonto (Account)* , *Kontaktperson* ,
*Lead* , *Lieferant/Partner* , *Verband* , *Aktivität* , *Dokument* . Diese sind durch Relationen verknüpft (siehe Abschnitt
...
Bereiche für *Alle Kontakte* , *Leads* , *Lieferanten* usw. – oder man nutzt Filter innerhalb einer Liste. Beim Klick
...
– daher eigener Weg.
- *Beziehungsmanagement:* Eine Alternative war, das Thema Verbände/Multiplikatoren zunächst wegzulassen
...
- *Dubletten-Handling:* Hier gab es Alternativen beim Merge: vollautomatisch (System entscheidet) vs.
...
- *Datenfelder Umfang:* Best Practice sagt “so viel wie nötig, so wenig wie möglich”. Wir haben uns
...
bleibt.
- *Offline-Umsetzung:* Alternative Ansätze waren diskutiert: Komplett offline-fähige App mit lokalem DB-Cache
...
**Vertrieb / Opportunity-Management:** *Übergabe:* Leads und Kontakte werden vom Kontaktmodul
...
.
*Verantwortung:* Außendienst/Vertriebsinnendienst pflegen die Felder, die für Opportunities relevant
...
*Abhängigkeit:* Die Opportunity-Pipeline hängt von aktuellen Kontaktdaten ab; umgekehrt hat
...
für Logistik und Montageplanung verwendet. *Verantwortung:* Projektmanager (PLAN) können den
Lieferstandort bei Bedarf ändern oder einen projektspezifischen Ort anlegen (z.B. temporäre Baustelle).
*Abhängigkeit:* Projekt benötigt gültige Location-Referenz oder fällt auf Rechnungsadresse des Kunden
...
(Projektcontrolling, Rechnungsplan) und **externe FiBu-Software (Lexware)** . *Übergabe intern:* Die
Rechnungslegung im System zieht die **Rechnungsadresse vom Kundenkontakt** . Ebenso fließen
Kundendaten ins Projektcontrolling (Umsatz je Kunde, etc.). *Verantwortung:* Buchhaltung muss vor
...
menschlich. *Übergabe extern:* Wir richten eine Schnittstelle ein, sodass **Kundenstammdaten und**
**Rechnungsdaten regelmäßig an Lexware übertragen**
werden
. Verantwortlich ist
Buchhaltung, diesen Abgleich (z.B. monatlich) zu initiieren. *Abhängigkeit:* Das CRM vergibt
...
Nummern-Duplikat geben, daher strenge Koordination. *Eskalation:* Sollte Sync fehlschlagen (z.B.
...
- *IT/Admin:* verwaltet Rechte, Backups, sorgt für DSGVO-konforme Löschkonzepte, etc., aber inhaltlich wenig
  Eingriff.
...
Suche (≤1s lokal via Lunr.js). *Maßnahme:* PouchDB/IndexedDB als lokale Datenbank; **Speicher-Limits beachtet** (iOS 50MB Design-Constraint, ADM=31MB passt ✅, siehe NFR_SPECIFICATION.md §4.1); 10 definierte Offline-Testszenarien (siehe NFR_SPECIFICATION.md §9: 7-Tage-Offline, Konflikte, Storage-Limit, Sync-Fehler, etc.).
...
*Annahme:* Globalisierung ist vorerst kein Thema (primär deutschsprachig)
, sodass wir uns auf
...
.
*Maßnahme:* Implementierung OWASP-geprüft, regelmäßige Berechtigungsaudits. Zudem physische
...
(kein Portal, kein ungesicherter Export). *Maßnahme:* Zusammenarbeit mit Datenschutzbeauftragtem
...
nicht einfach komplett löschen (erst nach Archivierung/Storno). *Maßnahme:* Archivmodus für
...
Daten aktuell zu halten (z.B. “Kunde seit 3 Jahren inaktiv” Markierung). *Maßnahme:* Erinnerungen
...
*Ursachen:* Gewohnheit, Anfangsaufwand, evtl. Performanceprobleme. *Mitigation:* Intensive
...
*Mitigation:* Vor Projektstart definieren wir eine Migrationsstrategie. Wahrscheinlich: Export aus
Outlook, Excel, Word durch Skripte, Bereinigung, Import. *Risikoaspekt:* mögliche **Datenverluste oder**
...
was im MVP sein muss und was warten kann. *Mitigation:* **Priorisierung strikt einhalten** – Muss-
...
als technisch anspruchsvoll. *Mitigation:* Evaluierung bewährter Techniken (z.B. PouchDB/CouchDB or
...
*Entscheidung:* MVP nutzt minimal viable Integration (CSV-Export). Lexware REST API wurde recherchiert und dokumentiert (verfügbar, gut dokumentiert, OAuth 2.0). Phase 2 Implementation nach MVP-Validierung geplant (4-6 Wochen Aufwand, €20-30k Budget). 
*Benefit:* MVP-Risiko eliminiert, Timeline um 4-6 Wochen verkürzt, Core-CRM-Validierung vor Integrationskomplexität. Händische Export/Import-Workflows in Phase 1 akzeptiert (wöchentlich 30-60 Min Aufwand Buchhaltung).
*Status:* Nicht mehr GoLive-gefährdend – deshalb ist es
Soll, nicht Muss, den vollen Sync zu haben
. Wichtig: in diesem Fall prozessual abfangen, damit
keine Rechnungen vergessen gehen.
**Compliance-Risiko:** DSGVO/GoBD-Fehler könnten teuer werden (Strafen). *Mitigation:* Externe
...
müssen zusammenpassen. *Mitigation:* Agile Entwicklung mit bereichsübergreifenden Teams,
...
**Kontakt-Datenpflege-Rollen** – *Platzhalter:* <Datenpflege-Rollen> – **Frage:** Wer soll im Betrieb
...
**Umgang mit Verbänden/Netzwerken** – *Platzhalter:*
...
*(Jeder offene Punkt wurde so präzise wie möglich formuliert. Die vorgeschlagenen Lösungen sind konform mit*
*dem Projektkontext und sollen als Diskussionsgrundlage dienen, um eine endgültige Entscheidung*
*herbeizuführen. Ziel ist, keine offenen Fragen in die Entwicklungsphase mitzunehmen.)*
...
*Außendienstmitarbeiter (Vertrieb Ladenbau)* – lieferte tiefe Einblicke in den Tagesablauf des ADMs und
...
*Innendienst & Kalkulation* – zeigte die Probleme mit verteilten Dateien und den Wunsch nach **“einer**
...
.
*Buchhaltung* – definierte Anforderungen wie **Rechnungspläne statt Zuruf**
und
Provisionsdokumentation
. Unsere Lösung zur Kontaktquelle speist sich daraus.
*Planungsabteilung* – Fokus lag auf Projektdurchführung, aber es wurde deutlich, dass **Briefing-**
**Übergaben** und Infoflüsse verbessert werden müssen
. Dies floss in Schnittstellen und
Persona-Mapping ein.
*Geschäftsführer* – braucht Übersicht (im Konzept erwähnt: **KPI-Dashboards**
). Auch strategische
...
*(Alle Quellen stammen aus dem bereitgestellten Projektkontext, es wurden keine externen Informationen darüber*

*hinaus verwendet. Die eckigen Klammer-Zitate* *【】* *verweisen auf die jeweiligen Stellen in den Dokumenten: [1]*
*Interview-Transkript, [2] Gesamtkonzept-PDF, [3–6] Persona-PDFs etc., jeweils mit Zeilenangaben. Diese dienen der*
...
### Persona-Bericht_ Buchhaltung (Integriertes CRM- und
...
Strategische Referenzpersona_ Planungsabteilung.pdf
